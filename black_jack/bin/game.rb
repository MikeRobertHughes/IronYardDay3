...
if answer == "yes" || answer == "y"
  while answer == "yes" || answer == "y"
    ...
    puts "The dealer is showing: #{dealer.show_hand}."
    puts "You have a #{player.show_hand} for a total of #{player.hand_value}."
    if player.hand_value == 21
      player.win(player.hand_value)
    end
    deal = nil
    while player.hand_value < 21 && (deal != "s" && deal != "stay") && player.hand.count < 6
      puts "Wanna (H)it that or (S)tay bro?"
      deal = gets.chomp.downcase
      if deal == "h" || deal == "hit"
        player.accept_card(deck.deal_card)
        puts "You now have a #{player.show_hand} for a total of #{player.hand_value}."
        if player.hand_value > 21
          ...
          puts "You have six cards and are under 21! You win bro-daddy!"
          system ("say You have six cards and are under 21! You win bro daddy!")
          break
        end
        ...
        while dealer.hand_value < 16
          ...
          puts "The dealer now has a #{dealer.show_hand} for a total of #{dealer.hand_value}."
          system('sleep .5')
        end
